...
async function walkHtml(dir, prefix = '') {
  ...
  for (const item of items) {
    ...
    if (item.isDirectory()) {
      ...
      // rel is already relative to process.cwd() root; dir is the cumulative
      // directory path and must NOT be prepended again (it would duplicate
      // segments, e.g. de/products/products/xxx.html).
      out.push({ path: rel, mtime: stat?.mtime });
    }
  }
  return out;
}
